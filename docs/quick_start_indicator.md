...
In this example, we will document an indicator produced by the World Bank, and published in the Bank's Poverty and Inequality Platform (PIP) and World Development Indicators (WDI): the *Poverty headcount ratio at $3.00 a day (2021 PPP) (% of population)*. The objective here is to document the indicator for cataloguing. This quick start focuses on **metadata**; for importing observation data, see [Observation data](/documenting_indicator_import_data.html).

The metadata we will enter in the Metadata Editor is the metadata published by the World Bank at https://data.worldbank.org/indicator/SI.POV.DDAY?locations=US. We will complement this metadata with information extracted from the data itself, e.g., to obtain the geographic and time coverage. Last, we will document the fact that this indicator is one of the Sustainable Development Goals monitoring indicators.

Example files for this indicator are described in [quick_start_files/indicator/README.md](/quick_start_files/indicator/README.html). For this quick start you need the thumbnail image (*poverty_thumbnail.jpg*) and the wide-format CSV [*SI.POV.DDAY_countries_data.csv*](/quick_start_files/indicator/SI.POV.DDAY_countries_data.csv) for geographic coverage. Long-format files are used when you [import data](/documenting_indicator_import_data.html) or [create a data structure from CSV](/managing_data_structures.html#create-a-data-structure-from-a-csv-file).

> This Quick start covers **metadata only** (no CSV import). For field definitions see [Reference metadata](/documenting_indicator_descriptive_metadata.html). For the full reference guide see [Documenting indicators](/documenting_indicator.html) (and [Documenting a database or dataset](/documenting_dataset.html) for indicator databases).
...
Documenting an indicator consists of entering information (metadata) about the indicator in metadata entry forms defined by a *metadata template*. When you create a new project, a default template is automatically selected. Use the **Indicator Schema 1.0 EN** or **IHSN INDICATOR 1.0 Template v01 EN** template (or your organization's indicator template). If the correct template is already selected, no action is needed. Otherwise, click the template name in the *Templates* frame, choose an indicator template, then click **Apply**. 
...
- ***Time Coverage***: 1960 to 2025 (from WDI extract; see example CSV)
...
For definitions of each field, see [Reference metadata](/documenting_indicator_descriptive_metadata.html).
...
- ***Export SDMX CSV***
  ...
  Export imported observation data in SDMX CSV format (available when observation data have been imported; see [Observation data](/documenting_indicator_import_data.html)).


- ***Export RDF/XML** and **Export RDF/JSON***
...
- ***Publish to NADA***
  ...
  To also publish a **data structure (DSD)** and **observation data** to NADA, see [Publish to NADA — Indicators](/publish_to_nada.html#indicators-dsd-and-observation-data) and [Export and publish](/documenting_indicator_export_publish.html).
