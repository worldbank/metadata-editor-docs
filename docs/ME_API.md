...
The Metadata Editor API can be used with any language that supports HTTP requests. To help users leverage the API without needing deep programming expertise, we have developed specialized open-source packages for R and Python users. These packages are available on GitHub and provide a more accessible entry point to the API's capabilities.
