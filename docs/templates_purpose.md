...
- ***Validation rules:*** Customized validation rules can be set for each metadata element, using regular expressions, length limits, date formats, or other checks, to guarantee metadata coherence and uniformity. See [Validating metadata](/validating_metadata.html#template-user-defined-validation-rules).
